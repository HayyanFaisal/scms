...
import { FileSpreadsheet, Download, History, Filter } from "lucide-react";
import { useGrantsWithDetails } from "@/hooks/useDatabase";
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList
          className={`grid w-full ${canViewAudit ? "grid-cols-2" : "grid-cols-1"}`}
        >
...
function PayrollExport() {
  ...
  const filteredGrants = grants.filter((grant) => {
    ...
    const isExpiring =
      approvedTo <= new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000) &&
      isActive;
    ...
    if (
      categoryFilter !== "all" &&
      grant.child?.Disability_Category !== categoryFilter
    )
      return false;
    return true;
  });

  const totalAmount = filteredGrants.reduce(
    (sum, g) => sum + g.Monthly_Amount,
    0,
  );
  const totalCFY = filteredGrants.reduce(
    (sum, g) => sum + (g.Total_CFY_Amount || 0),
    0,
  );
    ...
    const rows = filteredGrants.map((g) => [
      ...
      g.banking?.Bank_Name ||
        (g.banking?.Bank_Name_Branch &&
          g.banking?.Bank_Name_Branch.split(",")[0]),
      g.banking?.Branch_Address ||
        (g.banking?.Bank_Name_Branch &&
          g.banking?.Bank_Name_Branch.split(",")[1]),
    ...
    <>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Active grants with banking details for disbursement
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  filteredGrants.map((grant, index) => (
                    <TableRow key={index}>
                      <TableCell className="font-mono">
                        {grant.parent?.P_No_O_No}
                      </TableCell>
                      <TableCell>{grant.parent?.Parent_Name}</TableCell>
                      <TableCell>
                        {grant.child
                          ? formatChildDisplayName(
                              grant.child.Child_Name,
                              grant.parent?.Parent_Name,
                              grant.parent?.P_No_O_No,
                            )
                          : ""}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant="outline"
                          className={
                            grant.child?.Disability_Category === "A"
                              ? "border-red-400 text-red-600"
                              : grant.child?.Disability_Category === "B"
                      ...
                      <TableCell>
                        {grant.banking?.Bank_Name ||
                          (grant.banking?.Bank_Name_Branch &&
                            grant.banking?.Bank_Name_Branch.split(",")[0])}
                      </TableCell>
                      <TableCell className="font-mono">
                        {formatCurrency(grant.Monthly_Amount)}
                      </TableCell>
                      <TableCell className="font-mono">
                        {formatCurrency(grant.Total_CFY_Amount || 0)}
                      </TableCell>
