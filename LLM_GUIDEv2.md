...
The first Phase 4 banking workflow is implemented by migrations `015_banking_evidence_workflow.js` and `016_default_banking_evidence.js`, `server/banking-workflow.js`, and the staff **Banking Review** workspace. A parent maintains one active versioned banking record, uploads evidence using the same protected Phase 2 document store, submits it, and can receive a correction/rejection reason in the record-scoped conversation. Any account-detail edit clears verification and sets a new evidence freshness boundary; older files remain in history but cannot satisfy the new review. Staff need the separate `banking.verify` permission, are constrained by the banking data scope, and cannot verify until every current required banking document has separately reached `verified`. Verification, correction, rejection, archiving, account snapshots, actor, reason, and timestamps are retained in `scms_banking_history` and the general audit feed. The default published **Bank Account Evidence** requirement is editable through Configuration. This remains an evidence workflow only; it does not contact a bank.

The second Phase 4 increment is implemented by migration `017_payment_operations.js`, `server/payment-operations.js`, and the staff **Payments** workspace. An operator previews one authority and calendar month before creating a batch. Eligibility requires an active approved grant covering that entire month, approved parent/child records, a positive captured grant rate, and a verified active banking record. Every exclusion is returned with an explicit reason. Creation rechecks and locks eligibility in one transaction, stores immutable grant/banking snapshots, and places a unique grant/month guard on each line. A cancelled unexported batch releases the guard but retains its history. Fiscal budgets are authority/program/year records; only confirmed budgets authorize approval. The preparer cannot approve their own batch, and approval locks the budget row so concurrent batches cannot oversubscribe it. Sensitive formula-safe CSV export is separately permissioned and audited with a SHA-256 checksum. Paid, failed, and returned attempts are immutable and can be retried on the same line. No bank transfer is performed by SCMS.
...
Current completion: items 1 and 2 are implemented at the application-workflow level. Item 2 deliberately supports file-based bank handoff and result confirmation, not direct bank integration. Items 3 and 4 still require the detailed school/gadget workflows and their final role/scope-aware reporting views.
