...
The same generator creates `test-data/SCMS_Active_Grant_Import_Edge_Cases.xlsx`. After `npm run reset:demo-data`, choose the **Mixed parent and child rows** profile, the **Grant Edge Cases** sheet, and header row **1**. One row targets a seeded child with an active grant and must produce `financial_dependency`; another targets an existing child without a grant and must produce `child_already_exists`. A demographic import cannot change a captured rate, approved category, or grant period.

The import configuration page shows both mapping layers. **Built-in automatic heading map** is read-only software behavior and lists every bundled heading alias and canonical destination. **Recognized heading aliases** contains organization-specific additions maintained by authorized users. Matching ignores capitalization, spacing, punctuation, and underscores, but every suggestion remains reviewable before dry validation.

**Original upload retention** means how long the private server-side copy of a completed import spreadsheet is kept. Cleanup removes only that original upload after the configured number of days; it does not remove imported records, staging outcomes, downloadable results, logs, or audit history.
